...
void setup()
{
     ...
     attachInterrupt(pbIn, stateChange, FALLING); // Set the interrupt function, interrupt pin is digital pin D2, 
                                                    //interrupt service function is stateChange (), when the D2 power 
...
Serial.println(number); // Printing the number of times of interruption, which is convenient for debugging.
    delay(1);
    if(state == HIGH)  //When a moving object is detected, the ledout is automatically closed after the light 2S,
                         //the next trigger can be carried out, and No need to reset. Convenient debugging.
    {
        delay(2000);
        state = LOW;
...
{
    if(number>3)  //If in the set of the interrupt time the number more than 2 times, then means have detect moving objects,
                   //This value can be adjusted according to the actual situation, which is equivalent to adjust the threshold 
                   //of detection speed of moving objects.
         {
                   state = HIGH;
